...
// Fix by zbx, Standardize the path and use a left slash/, otherwise there will be issues compiling on Windows.
val jsiAbsolutePath = jsiDir.absolutePath.replace("\\", "/") // 规范路径，采用左斜杠/，否则在windows编译有问题
...
//              Fix by zbx, change the generator
//              if (Os.isFamily(Os.FAMILY_WINDOWS)) "-GNMake Makefiles" else "",
//              if (Os.isFamily(Os.FAMILY_WINDOWS)) "-GVisual Studio 16 2019" else "",
              if (Os.isFamily(Os.FAMILY_WINDOWS)) "-GVisual Studio 15 2017 Win64" else "",
              "-S",
              ".",
              "-B",
              hermesBuildDir.toString(),
              "-DJSI_DIR=" + jsiAbsolutePath))
    }
